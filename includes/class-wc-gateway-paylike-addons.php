...
class WC_Gateway_Paylike_Addons extends WC_Gateway_Paylike {
	...
	public function update_failing_payment_method( $subscription, $renewal_order ) {
		$subscription->update_meta_data('_paylike_transaction_id', $renewal_order->get_meta('_paylike_transaction_id', true ) );
		$subscription->update_meta_data( '_paylike_card_id', $renewal_order->get_meta('_paylike_card_id', true ) );
	...
	public function delete_resubscribe_meta( $resubscribe_order ) {
		$resubscribe_order->delete_meta_data('_paylike_transaction_id' );
		$resubscribe_order->delete_meta_data('_paylike_card_id' );
	...
	public function maybe_render_subscription_payment_method( $payment_method_to_display, $subscription ) {
		...
		$transaction_id = $subscription->get_meta('_paylike_transaction_id', true );
		...
		$payment_meta[ $this->id ] = array(
			'post_meta' => array(
				'_paylike_transaction_id' => array(
					'value' => $subscription->get_meta('_paylike_transaction_id', true ),
					'label' => 'A previous transaction ID',
				),
				'paylike_card_id'         => array(
					'value' => $subscription->get_meta('_paylike_card_id', true ),
	...
	protected function save_transaction_id( $result, $order ) {
		...
		foreach ( $subscriptions as $subscription ) {
			$subscription->update_meta_data('_paylike_transaction_id', $result['id'] );
			$subscription->save_meta_data();
		}
	}
	...
	protected function save_card_id( $card_id, $order ) {
		...
		foreach ( $subscriptions as $subscription ) {
			$subscription->update_meta_data('_paylike_card_id', $card_id );
			$subscription->save_meta_data();
		}
	}
}
