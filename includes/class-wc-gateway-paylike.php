...
class WC_Gateway_Paylike extends WC_Payment_Gateway {
	...
	function handle_authorize_result( $result, $order, $amount = 0 ) {
		...
		if ( is_wp_error( $result ) ) {
			...
			$order->update_meta_data('_paylike_transaction_captured', 'no' );
	...
	function handle_capture_result( $transaction, $order, $amount = 0 ) {
		...
		if ( is_wp_error( $result ) ) {
			...
			$this->save_transaction_id( $result, $order );
			$order->update_meta_data( '_paylike_transaction_captured', 'yes' );
			$order->save_meta_data();
			$order->payment_complete();
		}
	}
	...
	protected function save_transaction_id( $transaction, $order ) {
		...
		$order->update_meta_data( '_paylike_transaction_id', $transaction['id'] );
		$order->save_meta_data();
	}
	...
	protected function save_card_id( $card_id, $order ) {
		$order->update_meta_data( '_paylike_card_id', $card_id );
		$order->save_meta_data();
	}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		$order = wc_get_order( $order_id );
		$transaction_id = $order->get_meta('_paylike_transaction_id', true );
		$captured = $order->get_meta('_paylike_transaction_captured', true );
	...
	protected function get_card_id( $order ) {
		$card_id =  $order->get_meta('_paylike_card_id', true );
		if ( $card_id ) {
			return $card_id;
		}
		...
		foreach ( $subscriptions as $subscription ) {
			$card_id = $subscription->get_meta('_paylike_card_id', true );
			if ( $card_id ) {
				return $card_id;
			}
		}
